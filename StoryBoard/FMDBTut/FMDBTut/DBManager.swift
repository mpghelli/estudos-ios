...
//  FMDBTut
//
//  Created by Trainning on 06/02/21.
...
class DBManager: NSObject {
    ...
    override init(){
        ...
        pathToDabase = documentsDirectory.appending("/\(databaseFileName)")
    ...
    func createDatabase() -> Bool {
        ...
        if !FileManager.default.fileExists(atPath: pathToDabase)
        {
            database = FMDatabase(path: pathToDabase!)
        }
    ...
    func insertMovieData(){
        ...
        if openDatabase(){
            ...
            if let pathToMoviesFile = Bundle.main.path(forResource: "movies", ofType: "tsv"){
                ...
                do{
                    let moviesFilesContents = try String(contentsOfFile: pathToMoviesFile)
                    let moviesData = moviesFilesContents.components(separatedBy: "\r\n")
...
//    }
    ...
    func updateMovie(withID ID: Int, watched: Bool, likes: Int) -> Void {
        if openDatabase(){
            let query = "update movies set \(field_MovieWatched)=?, \(field_MovieLikes)=?"
